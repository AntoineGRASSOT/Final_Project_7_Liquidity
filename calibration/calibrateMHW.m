...
%
%  Minimises eq. (4.1):  Err^2 = sum_{i=1}^9 [R^{C,MHW}_i - R^{C,mkt}_i]^2
%  over p = (a, sigma, gamma) on co-terminal 10y diagonal:
%  1y9y, 2y8y, ..., 9y1y.
%
%  Inputs (project bootstrap format):
%    data      - readExcelData_OIS output  (uses data.vols, bps)
%    dates     - readExcelData_OIS output  (uses dates.settlement)
...
%% ── 2. Market vols  ──────────────────────────────────────────────────────
%  data.vols may have a leading NaN (empty header cell in Excel).
%  We skip leading NaN entries and take exactly M = 9 finite values.

vols_all = data.vols(:);
idx0     = find(isfinite(vols_all), 1, 'first');   % first finite position
...
vol_bps = vols_all(idx0 : idx0+M-1);   % (9×1) bps  — never reassigned below
vol_dec = vol_bps / 1e4;               % decimal

fprintf('   Vol index: data.vols(%d:%d)\n', idx0, idx0+M-1);
fprintf('   Vols (bps):'); fprintf(' %6.2f', vol_bps); fprintf('\n');

%% ── 3. Market prices  R^{C,mkt}  (Bachelier ATM, eq. B.4) ───────────────
p_mkt    = zeros(M,1);
S_vec    = zeros(M,1);
B_vec    = zeros(M,1);
C_vec    = zeros(M,1);
...
for i = 1:M
    ...
    p_mkt(i) = bachelierCS(S0, S0, expiry_y(i), B_a, C0, vol_dec(i));
...
for i = 1:M
    ...
    vi_bps(i) = bachelierCS_implVol(S_vec(i), S_vec(i), expiry_y(i), ...
...
%% =========================================================================
%%  LOCAL FUNCTION – NaN-safe objective  (eq. 4.1, Baviera 2019)
%% =========================================================================
