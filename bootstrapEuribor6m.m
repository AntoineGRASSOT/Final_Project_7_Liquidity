...
for k = 7:11
    t_k  = t_hat(k);
    P_k  = interpolateOIS(t0, [t6; t12], [P_6; P_12], t_k);
    knot_dates = [knot_dates; t_k];
    knot_df    = [knot_df;    P_k];
    z_k = -log(P_k) / yearfrac(t0, t_k, 3) * 100;
    fprintf('    t̂_%d: P = %.8f  z = %.4f%%\n', k, P_k, z_k);
end

[knot_dates, si] = sort(knot_dates); % Sort knots so far
knot_df = knot_df(si);

% STEP 3b: Backward move with FRAs 1x7, 2x8, ..., 5x11
%         P(t̂_i) = P(t̂_{i+6}) * (1 + delta(t̂_i, t̂_{i+6}) * F_FRA)
%
% Now P(t̂_{i+6}) for i=1..5 means P(t̂_7)...P(t̂_11), just interpolated in Step 3a.

for i = 1:5
    t_i    = t_hat(i);      % t̂_i unknown
    t_ip6  = t_hat(i+6);    % t̂_{i+6} known

    % P(t̂_{i+6}) already on knot grid from Step 3a
    P_ip6 = interpolateOIS(t0, knot_dates, knot_df, t_ip6);

    % Find FRA i x (i+6): start ~= t̂_i, end ~= t̂_{i+6}
    [~, idx_fra] = min(abs(fra_start - t_i));
    F_i          = fra_mid(idx_fra);
    delta_i      = yearfrac(t_i, t_ip6, 2);    % Act/360

    % Backward formula: P(t̂_i) = P(t̂_{i+6}) * (1 + delta * F)
...
% Spline for S_18m
t_IRS_yrs = yearfrac(t0, IRS_endDates, 3);   % Act/365 for spline axis
t_18m_yrs = yearfrac(t0, t_hat18, 3);
S_18m = interp1(t_IRS_yrs, IRS_rates, t_18m_yrs, 'spline');
fprintf('    S_18m (spline) = %.6f%%\n', S_18m*100);

% Fixed leg of 18m swap
% Schedule: t0 -> t_1y (full year) -> t_18m (6m stub)
fix_dates_18m  = [IRS_endDates(2); t_hat18];   % [t_1y ; t_18m]
fix_starts_18m = [t0; IRS_endDates(2)];
delta_fix_18m  = yearfrac(fix_starts_18m, fix_dates_18m, 6);  % 30/360
PD_fix_18m     = interpolateOIS(t0, OIS_dates, OIS_df, fix_dates_18m);
annuity_18m    = sum(delta_fix_18m .* PD_fix_18m);
I1 = S_18m * annuity_18m;

% Floating leg weights w_k
% t̃_1=t6, t̃_2=t12, t̃_3=t18  for the 18m swap
flt_dates_18m  = [t6; t12; t_hat18];
flt_starts_18m = [t0; t6;  t12];
delta_flt_18m  = yearfrac(flt_starts_18m, flt_dates_18m, 2);  % Act/360
PD_flt_18m = interpolateOIS(t0, OIS_dates, OIS_df, flt_dates_18m);
w_18m = delta_flt_18m .* PD_flt_18m;   % w1, w2, w3

% Known forwards F_1, F_2
delta_01  = yearfrac(t0, t6,  2);   % Act/360, floating leg
delta_612 = yearfrac(t6, t12, 2);
F_1 = (1.0/P_6  - 1.0) / delta_01;
F_2 = (P_6/P_12 - 1.0) / delta_612;
fprintf('    F_1 = %.6f%%,  F_2 = %.6f%%\n', F_1*100, F_2*100);

% F_3 from 18m : 2*w_3*F_3 = I(1) - w_1*F_1 - w_2*F_2
w3  = w_18m(3);
F_3 = (I1 - w_18m(1)*F_1 - w_18m(2)*F_2) / (2.0 * w3);
fprintf('    F_3 = %.6f%%\n', F_3*100);

% P(t̂_18) from F_3
delta_12_18 = yearfrac(t12, t_hat18, 2);   % Act/360, floating period
...
% 2y swap: I(2) -> F_4
% Fixed leg of 2y IRS: ANNUAL, 30/360
% Schedule: t0 -> t_1y -> t_2y
fix_dates_2y  = [IRS_endDates(2); IRS_endDates(3)];   % t_1y, t_2y
fix_starts_2y = [t0; IRS_endDates(2)];
delta_fix_2y  = yearfrac(fix_starts_2y, fix_dates_2y, 6);  % 30/360
PD_fix_2y     = interpolateOIS(t0, OIS_dates, OIS_df, fix_dates_2y);
annuity_2y    = sum(delta_fix_2y .* PD_fix_2y);
I2 = IRS_rates(3) * annuity_2y;

% w_4: floating leg period [t̂_18 -> t_2y], Act/360
t_2y   = IRS_endDates(3);   % t_2y = 14-Sep-2017
PD_t2y = interpolateOIS(t0, OIS_dates, OIS_df, t_2y);
w4     = yearfrac(t_hat18, t_2y, 2) * PD_t2y;
F_4    = (I2 - I1) / w4;

% P(t_2y) from F_4
P_2y = P_18 / (1.0 + yearfrac(t_hat18, t_2y, 2) * F_4);
...
for yr = 3:12    % 3y to 12y
    ...
    delta_fix_i  = yearfrac(fix_starts_i, fix_schedule(2:end), 6); % 30/360
...
E6m_dates = knot_dates(mask); E6m_df    = knot_df(mask);
